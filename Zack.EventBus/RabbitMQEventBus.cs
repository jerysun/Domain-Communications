...
class RabbitMQEventBus : IEventBus, IDisposable
{
        ...
        IServiceScopeFactory serviceProviderFactory, string exchangeName, string queueName)
    {
        ...
        //大部分是Scoped，因此必须这样显式创建一个scope，否则在getservice的时候会报错：Cannot resolve from root provider because it requires scoped service
    ...
    private async Task ProcessEvent(string eventName, string message)
    {
        if (_subsManager.HasSubscriptionsForEvent(eventName))
        {
            ...
            foreach (var subscription in subscriptions)
            {
                ...
                if (handler == null)
                {
                    throw new ApplicationException($"Fail to create the service of type {subscription}");
        }
        ...
        else
        {
            ...
            if (entryAsm != null)
            {
                Debug.WriteLine($"There's no event handler for the event '{eventName}'，entryAsm:{entryAsm}");
            }
        }
    }
}